...
public class Pathfinder : MonoBehaviour
{
    ...
    private void InitNodes(uint x, uint y)
    {
        ...
        for (int i = 0; i < x; i++)
        {
            for (int j = 0; j < y; j++)
            {
                Tuile t_Tuile = m_Grid.GetTuile(new Vector2Int(i, j));
    ...
    public Path GetPath(Transform a_StartPoint, Tuile a_EndTile, bool a_DiagonalAllowed = true)
    {
        Vector2Int t_startGridPoint = m_Grid.WorldToGrid(a_StartPoint.position);
        Tuile t_startTile = m_Grid.GetTuile(t_startGridPoint);
        ...
        Node t_StartNode = m_nodes[t_startTile.x, t_startTile.y];
